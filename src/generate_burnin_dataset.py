...
def build():
    ...
    for lot_id in range(1, N_LOTS + 1):
        ...
        for i in range(PARTS_PER_LOT):
            ...
            cls = draw_class(prof)
            # one shared degradation "health factor" per part, plus
            # per-parameter modulation -> parameters are correlated, which
            # is what makes multivariate detection pay off
            A_part, n_part = drift_coeffs(cls)
            # a real defect mechanism shows up in ONE or TWO parameters, not
            # all four -- so the signature is a correlation break, and a
            # detector that only watches Iddq will miss half the population
            ...
            for p, cfg in PARAMS.items():
                ...
                for t in TIMEPOINTS:
                    ...
                    meas = true_v * (1.0 + RNG.normal(0.0, MEAS_NOISE))
    ...
    df.to_csv(f"{OUTDIR}/burnin_wide.csv", index=False)
    ...
    pd.DataFrame(long_rows).to_csv(f"{OUTDIR}/burnin_long.csv", index=False)
    ...
    train.to_csv(f"{OUTDIR}/public_train.csv", index=False)

    hidden_cols = [c for c in test.columns
                   if c.endswith("_168h") or c in ("true_class", "is_latent_defect",
                                                   "static_fail_168h")]
    test.drop(columns=hidden_cols).to_csv(f"{OUTDIR}/hidden_test.csv", index=False)
    test[["serial"] + hidden_cols].to_csv(f"{OUTDIR}/ground_truth.csv", index=False)
