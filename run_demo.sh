...
banner "1/7  Generate the dataset (seed 42, reproducible)"
"$PY" src/generate_burnin_dataset.py

banner "2/7  Tests"
"$PY" -m pytest tests/ -q

banner "3/7  Baseline - what Module A has to beat (reproduces blueprint S12)"
"$PY" -m src.baseline

banner "4/7  Diagnostics - is there joint structure? where is recall going?"
"$PY" -m src.diagnose_why

banner "5/7  Sensitivity - how much of the ceiling is metrology, not model?"
"$PY" -m src.sensitivity

banner "6/7  Full screen - fused verdict, PDA status, reason codes"
"$PY" -m src.report
